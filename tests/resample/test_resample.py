...
def test_resample_mostly_defaults(weight_type):
    ...
    resample = Resample(n_input_models=nmodels, output_wcs=output_wcs, weight_type=weight_type)
...
def test_resample_error_scaling(compute_err):
    ...
    assert np.allclose(np.nanmedian(odata), sb_in, atol=0, rtol=1e-6)
    # Surface brightness error should have scaled with pixel area
    assert np.allclose(np.nanmedian(oerr), sb_err_in * (pscale_in / pscale_out), atol=0, rtol=1e-6)
