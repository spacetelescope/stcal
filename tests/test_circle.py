...
def test_circle_scaling(trial):
    ...
    reference_circle = Circle(
        (test_circle.center[0] * scale, test_circle.center[1] * scale), test_circle.radius * abs(scale)
    )
    ...
    for i in range(len(points)):
        ...
        for j in range(i + 1, len(points)):
            ...
            for k in range(j + 1, len(points)):
                ...
                if (
                    c is not None
                    and (result is None or c.radius < result.radius)
                    and all(s in c for s in points)
                ):
                    result = c
