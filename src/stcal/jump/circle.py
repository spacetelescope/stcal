...
class Circle:
    ...
    def from_points(cls, points: list[tuple[float, float]]) -> "Circle":
        ...
        if len(points) == 0:
            ...
            radius = np.max(np.hypot(*(center - points).T))
    ...
    def __add__(self, delta: tuple[float, float]) -> "Circle":
        if isinstance(delta, float):
            delta = (delta, delta)
        return self.__class__(center=self.center + np.array(delta), radius=self.radius)

    def __mul__(self, factor: float) -> "Circle":
        return self.__class__(center=self.center, radius=self.radius + factor)

    def __contains__(self, point: tuple[float, float]) -> bool:
        return np.hypot(*(np.array(point) - self.center)) <= self.radius * RELATIVE_TOLERANCE
    ...
    def almost_equals(self, other: "Circle", delta: Optional[float] = None) -> bool:
        ...
        return np.allclose(self.center, other.center, atol=delta) and np.allclose(
            self.radius, other.radius, atol=delta
        )
    ...
    for point_index, point in enumerate(points):
        if point not in circle:
            if circle.radius == 0.0:
                ...
                circle = _expand_circle_from_two_points(
                    known_boundary_point, point, points[: point_index + 1]
                )
    return circle
...
def _expand_circle_from_two_points(
    ...
    known_boundary_points = np.array([known_boundary_point_a, known_boundary_point_b])
    ...
    for point in points:
        ...
        if circumcircle_cross_product > 0.0 and (
            left is None
            or circumcenter_cross_product > _triangle_cross_product((*known_boundary_points, left.center))
        ):
            left = circumcircle
        elif circumcircle_cross_product < 0.0 and (
            right is None
            or circumcenter_cross_product < _triangle_cross_product((*known_boundary_points, right.center))
        ):
            right = circumcircle
...
def circumcircle_from_points(
    ...
    incenter = (np.min(points, axis=0) + np.max(points, axis=0)) / 2
    ...
    return Circle(
        ...
        radius=np.max(np.hypot(*(relative_circumcenter - relative_points).T)),
    )


def _triangle_cross_product(
    ...
    return (triangle[1][0] - triangle[0][0]) * (triangle[2][1] - triangle[0][1]) - (
