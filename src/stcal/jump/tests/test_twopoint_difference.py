...
#from stcal.jump.constants import dqflags
from stcal.jump.constants import dqflags
from stcal.jump.constants import update_dqflags
dqflags = {
    "DO_NOT_USE": 1,
    "SATURATED": 2,
    "JUMP_DET": 4,
...
def test_4grps_1cr(setup_cube):
    ...
    out_gdq, row_below_gdq, row_above_gdq = find_crs(data, gdq, read_noise, rej_threshold,
                                                     rej_threshold, rej_threshold, nframes, False, 200, 10, dqflags)
...
def test_6grps_different_valid_grps_each_pixel(setup_cube):
    ...
    data[0, 4, 1, 0] = 2510
    data[0, 5, 1, 0] = 1522
    gdq[0, 4, 1, 0] = dqflags['SATURATED']
    gdq[0, 5, 1, 0] = dqflags['SATURATED']
    # pixel 1, 1 two crs
    data[0, 0, 1, 1] = 0
    data[0, 1, 1, 1] = 1000
    data[0, 2, 1, 1] = 1100
    ...
    data[0, 4, 2, 0] = 10102
    data[0, 5, 2, 0] = 10208
    gdq[0, 3, 2, 0] = dqflags['SATURATED']
    gdq[0, 4, 2, 0] = dqflags['SATURATED']
    gdq[0, 5, 2, 0] = dqflags['SATURATED']
...
def test_3diff_median_vector():
    ...
    median = calc_med_first_diffs(indiffs)
    assert median == 7
...
def test_5diff_1sat_median_vector():
    indiffs = np.asarray([7, 5, 10, 12, np.nan])
    median = calc_med_first_diffs(indiffs)
    assert median == 7


def test_5diff_2sat_median_vector():
    indiffs = np.asarray([7, 5, 10, np.nan, np.nan])
    median = calc_med_first_diffs(indiffs)
    assert median == 7
...
def test_4diff_median_array():
    ...
    indiffs = np.zeros(shape=(4, 2, 2),dtype=np.float32)
    indices = np.zeros(shape=(4, 2, 2),dtype=np.int32)
    indiffs[:, 0, 0] = [11, 6, 9, 13]
    indiffs[:, 0, 1] = [9, 4, 7, 3]
    indiffs[:, 1, 0] = [10, 5, 3, 12]
    indiffs[:, 1, 1] = [1, 3, 8, 12]
    med_array = calc_med_first_diffs(indiffs)
