...
def match(images, skymethod="global+match", match_down=True, subtract=False):
    ...
    images : list of SkyImage or SkyGroup
        A list of :py:class:`~stcal.skymatch.skyimage.SkyImage` or
    ...
    skymethod : {'local', 'global+match', 'global', 'match'}, optional
        ...
        * **'match'** : compute differences in sky values between images
          ...
          `astrodrizzle
          <http://stsdas.stsci.edu/stsci_python_sphinxdocs_2.13/drizzlepac/astrodrizzle.html>`_
    ...
    **Limitations and Discussions:**
    ...
    if do_match:
        ...
        sky_deltas = [skd if np.isfinite(skd) else None for skd in sky_deltas]
...
def _apply_sky(images, sky_deltas, do_global, do_skysub, show_old):
    for img, sky in zip(images, sky_deltas):
        ...
        if is_group:
            ...
            # apply sky change:
            old_sky = img.sky if img.sky is not None else 0
            if do_skysub:
