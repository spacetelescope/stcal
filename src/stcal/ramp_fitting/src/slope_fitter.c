...
/*
 * The input arguments for ols_slope_fitter API is essentially the elements of
 * the RampData data structure defined in ramp_data_class.py.
 *
 * These are the arguments passed to this function from the python code:
 *      4-D Arrays (ndarrays)
 * ramp_data.data
 * ramp_data.groupdq
 * ramp_data.err
 ...
 *
 *      Meta Information
 * ramp_data.frame_time
 * ramp_data.group_time
 * ramp_data.groupgap
 * ramp_data.nframes
 *
 *      Group and Pixel Flags
 * ramp_data.flags_do_not_use
 * ramp_data.flags_jump_det
 * ramp_data.flags_saturated
 * ramp_data.flags_no_gain_val
 * ramp_data.flags_unreliable_slope
 *
 * Weighting type for OLS algorithm.
 *
 * MAIN
 */
static PyObject *
ols_slope_fitter(PyObject * module, PyObject * args) {
...
/*
 * Add a segment to the segment list for the ramp.
 */
static int
add_segment_to_list(
        ...
        struct segment_list * segs)
{
    ...
    /* Clean each list for each integration */
        ...
        npy_intp integ)
{
    ...
    /* Prune segment list of length 1 segments if longer segments exist */
...
create_rateint_product(
        struct rateint_product * rateint,
        struct ramp_data * rd)
{
    const npy_intp nd = 3;
    npy_intp dims[nd];
